...
$jsonStatus = $Result | ConvertTo-Json -Compress
Write-Output $jsonStatus
#$Script:LogPath = "$LogFolder\WindowsUpdateComplianceTracker.log"
#Set-Content -Path $LogPath -Value $jsonStatus -ErrorAction SilentlyContinue


If ($isLatestForThisDevice -eq $true) {
	Exit 0
} Else {
	Exit 1
}
